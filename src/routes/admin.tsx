...
function AdminPage() {
  ...
  function sellerProfileOf(p: any) {
    return profiles.find((x) => x.id === p.seller_id);
  }
  ...
  if (loading || !rolesLoaded) {
    return (
      <AppShell>
        <div className="p-10 text-center text-muted-foreground text-sm">Loading admin…</div>
      </AppShell>
    );
  }
  if (!isAdmin)
  ...
  function buyerOf(o: any) {
    return profiles.find((p) => p.id === o.buyer_id);
  }
  function sellerOf(o: any) {
    return profiles.find((p) => p.id === o.seller_id);
  }

  const queue = products.filter(
    (p) => p.moderation_status !== "approved" && p.moderation_status !== "rejected",
  );
  ...
  async function setVerificationStatus(id: string, status: string, notes?: string) {
    ...
    const { error } = await (supabase as any)
      ...
      .update({
        status,
        admin_notes: notes ?? v?.admin_notes,
        reviewed_by: user!.id,
    ...
    <AppShell>
      <section className="bg-hero text-primary-foreground px-5 py-6 rounded-b-[2.5rem]">
        ...
        <p className="text-xs text-primary-foreground/80">
          Middleman dashboard · all contacts visible
        </p>
        <div className="grid grid-cols-4 gap-2 mt-4">
          {stats.map((s) => (
            <div
              key={s.label}
              className="bg-primary-foreground/15 backdrop-blur rounded-2xl p-2 text-center"
            >
      ...
      <div className="px-5 pt-6">
        <Tabs defaultValue="orders">
          <TabsList className="grid grid-cols-5 w-full h-auto">
            <TabsTrigger value="queue" className="text-[11px]">
              Queue
            </TabsTrigger>
          ...
          <TabsContent value="queue" className="space-y-2 mt-4">
            <p className="text-xs text-muted-foreground">
              Edit, then approve to publish. {queue.length} pending.
            </p>
            {queue.length === 0 && (
              <p className="text-center text-sm text-muted-foreground py-8">Queue empty 🌾</p>
            )}
            {queue.map((p) => {
              ...
              <Card key={v.id} className="p-3 shadow-card border-0 space-y-1">
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <div className="text-[11px] text-muted-foreground">
                      {v.contact_number} · {v.momo_network} {v.momo_number}
                    </div>
                  </div>
                  <span className="text-[10px] uppercase font-bold tracking-wide bg-secondary px-2 py-0.5 rounded-full">
                    {v.status}
                  </span>
                </div>
                <Button
              ...
              return (
                <Card key={o.id} className="p-3 shadow-card border-0 space-y-2">
                  ...
                  <div className="grid grid-cols-2 gap-2 text-[11px] bg-muted rounded-xl p-2">
                    <div>
                      <div className="font-semibold uppercase text-[9px] tracking-wide text-muted-foreground mb-1">
                        Buyer
                      </div>
                  ...
                  {o.notes && (
                    <p className="text-xs italic text-muted-foreground">Note: {o.notes}</p>
                  )}
            ...
            {products.map((p) => {
              ...
              return (
                <Card key={p.id} className="p-3 shadow-card border-0 flex gap-3 items-center">
                  ...
                  <span
                    className={`text-[9px] uppercase font-bold tracking-wide px-2 py-0.5 rounded-full ${p.available ? "bg-success/20 text-primary" : "bg-muted text-muted-foreground"}`}
                  >
      ...
      <ProductEditDialog
        ...
        onSave={moderateProduct}
      />
      <VerificationReviewDialog
        v={reviewV}
        onClose={() => setReviewV(null)}
        onAction={setVerificationStatus}
      />
    </AppShell>
  );
}
...
function ProductEditDialog({
  ...
  product: any;
  onClose: () => void;
  onSave: (id: string, status: string, patch?: any) => void;
}) {
        ...
        <div className="space-y-2 text-sm">
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <label>
              ...
              <Input
                type="number"
                value={data.quantity_available ?? 0}
                onChange={(e) => set("quantity_available", Number(e.target.value))}
              />
            </label>
          </div>
          <label className="block">
            <span className="text-xs">Moderation notes (internal)</span>
            <Textarea
              rows={2}
          ...
          <div className="flex gap-2 pt-2">
            ...
            <Button
              ...
              className="rounded-full"
              onClick={() =>
                onSave(data.id, "requires_changes", { moderation_notes: data.moderation_notes })
              }
            >
              Request Changes
            </Button>
    ...
    <Dialog open={!!v} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-w-sm rounded-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-2 text-sm">
          ...
          <Row
            label="Location"
            value={[v.district, v.parish, v.village].filter(Boolean).join(", ")}
          />
          ...
          <Row
            label="MoMo"
            value={`${v.momo_network ?? ""} ${v.momo_number ?? ""} (${v.momo_name ?? ""})`}
          />
          {v.farm_photos?.length || v.crop_photos?.length ? (
            <div className="grid grid-cols-3 gap-1 pt-2">
              {[...(v.farm_photos ?? []), ...(v.crop_photos ?? [])].map(
                (src: string, i: number) => (
          ...
          <label className="block pt-2">
            <span className="text-xs">Admin notes (visible to farmer)</span>
            <Textarea rows={2} value={notes} onChange={(e) => setNotes(e.target.value)} />
          </label>
          <div className="grid grid-cols-2 gap-2 pt-2">
            <Button className="rounded-full" onClick={() => onAction(v.id, "approved", notes)}>
              Approve
            </Button>
            <Button
              variant="outline"
              className="rounded-full"
              onClick={() => onAction(v.id, "info_required", notes)}
            >
              Request Info
            </Button>
            <Button
              variant="outline"
              className="rounded-full"
              onClick={() => onAction(v.id, "suspended", notes)}
            >
              Suspend
            </Button>
            <Button
              variant="destructive"
              className="rounded-full"
              onClick={() => onAction(v.id, "rejected", notes)}
            >
              Reject
            </Button>
          </div>
